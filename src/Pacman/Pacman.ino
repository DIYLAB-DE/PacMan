/**
* PACMAN GAME - PORTED TO TEENSY 4.0 / TEENSY 4.1
* ===============================================
*
* CREDITS. Parts of this code is based on:
* Pacman-Arduino-Due: <https://github.com/nathalis/Pacman-Arduino-Due>
...
// game variables ans definitions
enum GameState {
    ReadyState,
    PlayState,
...
#define C16(_rr,_gg,_bb) (((uint16_t)(((_rr & 0xE0)) | ((_gg & 0xE0) >> 3) | ((_bb & 0xC0) >> 6)) <<8 ))
uint16_t _paletteW[] = {
    C16(0,0,0),          // 0 black
    C16(255,0,0),        // 1 red
    C16(222,151,81),     // 2 brown
    C16(255,128,255),    // 3 pink
    C16(0,0,0),          // 4 black
...
public:
    ...
    byte GetTile(int16_t cx, int16_t ty) {
        if (_state != ReadyState && ty == 20 && cx > 10 && cx < 17) return 0;  // ready to next zone
    ...
    void PacmanAI() {
        ...
        Sprite* frightenedGhost = 0;
        Sprite* closestAttackingGhost = 0;
        Sprite* DeadEyesStateGhost = 0;
        int16_t dist = 0x7FFF;
    ...
    void MoveAll() {
        ...
        for (byte i = 0; i < 5; i++) {
            ...
            if (_state == DeadGhostState && s->state != DeadEyesState) continue;
            ...
            if ((x & 0x7) == 0 && (y & 0x7) == 0)  // cell aligned
                s->dir = ChooseDir(s->dir, s);  // choose another direction 
    ...
    void SetScoreChar(byte i, signed char c) {
        if (_scoreStr[i] == c) return;
        _scoreStr[i] = c;
        Mark(i + 32);  // score  
    }

    void SetHiScoreChar(byte i, signed char c) {
        if (_hiscoreStr[i] == c) return;
        _hiscoreStr[i] = c;
    ...
    void Step() {
        ...
        // create a bitmap of dirty tiles (144 bytes)
        byte m[(32 / 8) * 36];
        memset(m, 0, sizeof(m));
        _dirty = m;

        if (!GAMEPAUSED) MoveAll();  // if game paused stop all
...
void setup() {
    ...
    // clear display black
    for (int i = 0; i < 240 * 320; i++) fb[i] = 0;
    tft.update(fb, true);
}
...
void drawIndexedmap(const uint8_t* indexmap, int16_t x, int16_t y, uint16_t w, uint16_t h) {
    byte i = 0;
    uint16_t color = __paletteW[indexmap[0]];

    for (byte tmpY = 0; tmpY < 8; tmpY++) {
        byte width = 1;
        for (byte tmpX = 0; tmpX < 8; tmpX++) {
            uint16_t next_color = __paletteW[indexmap[++i]];

            if ((color != next_color && width >= 1) || tmpX == 7) {
                drawFastHLine(x + tmpX - width + 1, y + tmpY, width, color);
                color = next_color;
                width = 0;
            }
            width++;
...
void drawFastHLine(int x, int y, int w, uint16_t color) {
    if ((y < 0) || (y >= 320) || (x >= 240)) return;
    if (x < 0) { w += x; x = 0; }
    if (x + w > 240) { w = 320 - x; }
    uint16_t* p = fb + x + y * 240;
    while (w-- > 0) {
        (*p) = color;
        p++;
    }
